...
function LayerEvap(Layers, nodaysvector, ea_S, layerUH, NoL)
  ...
  LayersLast = zeros(nodaysvector[NoL],NoL)
  for i in 1: NoL
    LayersLast[1:nodaysvector[i],i].= Layers[1:nodaysvector[i],i]
  end
  ...
  for j in 1 : NoL                     # 1 is the top(fastest) Layer NoL is the bottom layer 
    ...
    if(redea > 0.0)
      ...
      if(sum(Layers) > 0.0)             
        newLayer .= Layers[1:nodaysvector[j],j]
        ...
        if (differ > 0.0) # the Layer has more water than is to be evaporated > ea_S                
            ea_excess = 0.0
            evapUH = redea .* layerUH[1:nodaysvector[j], j]
            newLayer .= Layers[1:nodaysvector[j],j] .- evapUH[1:nodaysvector[j]]  
            ...
            if(length(tull) > 0)
              ...
              newLayer .= Layers[1:nodaysvector[j],j]- evapUH[1:nodaysvector[j]] # updates so that everthing is positive
            end   

            if (round(sum(Layers[1:nodaysvector[j],j])-sum(newLayer)- redea+ea_excess; digits= 8)!= 0.0)        
              avvik = round(sum(Layers[1:nodaysvector[j],j]) -sum(newLayer)- redea+ea_excess; digits= 8)
        ...
        #println(sum(LayersLast))       
     Layers[1:nodaysvector[j],j] .= newLayer[1:nodaysvector[j]]  # updating the actual layer
        #println(sum(Layers))
        #println(sum(LayersLast))    
      else                           # sumLayers == 0    
        ea = 0.0
